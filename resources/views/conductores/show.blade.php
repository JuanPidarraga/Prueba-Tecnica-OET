...
@section('title', 'Detalles del Conductor - ACME Transportes')
...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex justify-between items-center">
        ...
        <div class="space-x-2">
            <a href="{{ route('conductores.edit', $conductor) }}" class="btn-primary">
                Editar
            </a>
    ...
    <div class="bg-white rounded-lg shadow-md p-8 mb-6">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">Detalles del Conductor</h1>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-500 mb-1">Número de Cédula</label>
                <p class="text-lg font-semibold text-gray-900">{{ $conductor->numero_cedula }}</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-500 mb-1">Nombre Completo</label>
                <p class="text-lg font-semibold text-gray-900">{{ $conductor->nombre_completo }}</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-500 mb-1">Ciudad</label>
                <p class="text-lg text-gray-900">{{ $conductor->ciudad }}</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-500 mb-1">Teléfono</label>
                <p class="text-lg text-gray-900">{{ $conductor->telefono }}</p>
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-500 mb-1">Dirección</label>
                <p class="text-lg text-gray-900">{{ $conductor->direccion }}</p>
            </div>
        </div>
    </div>

    <div class="bg-white rounded-lg shadow-md p-8">
        <h2 class="text-2xl font-bold text-gray-800 mb-4">Vehículos Asignados</h2>
        ...
        @else
            <p class="text-gray-500 text-center py-8">No hay vehículos asignados a este conductor</p>
        @endif
    </div>
</div>
@endsection
